...
if __name__ == '__main__':
    ...
    args.add_argument('-t', dest='to_addr', type=lambda x: val2list(x), default=[], required=True, help='Sent Address')
    args.add_argument('-f', dest='from_addr', type=lambda x: val2list(x), default=[], required=True, help='Sent From')
    ...
    args.add_argument('-T', '--type', dest='msg_type', choices=['txt', 'html'], default='html', help='Message Type')
    ...
    for sender in args.from_addr:
        for user in args.to_addr:
            try:
                smtp_relay(sender, user, args.subject, args.body, args.relay_srv, args.relay_port, args.auth_pwd,
