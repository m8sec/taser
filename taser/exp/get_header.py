...
def emit_output(parts, cli_logger, file_logger):
    with WRITE_LOCK:
        cli_logger.info(parts)
        file_logger.info('\t'.join(str(part) for part in parts))


def output_handler(resp, args, cli_logger, file_logger):
    if args.hide_title:
        emit_output([resp.url, highlight('Title', fg='gray'), '\t         ({})\t {}'.format(get_statuscode(resp), get_title(resp))], cli_logger, file_logger)

    if args.header:
        for x in delimiter2list(args.header, delim=","):
            h = extract_header(x, resp)
            if h:
                emit_output([resp.url, x+":", h], cli_logger, file_logger)
    elif args.verbose:
        for k, v in resp.headers.items():
            if k not in BLACKLIST:
                emit_output([resp.url, k+":", v], cli_logger, file_logger)
    else:
        for k, v in resp.headers.items():
            for w in DEFAULT:
...
def launcher(url, args, seen_urls, cli_logger, file_logger):
    resp = web_request(url, proxies=args.proxy, timeout=args.timeout)
    if get_statuscode(resp) != 0:
        with WRITE_LOCK:
            if resp.url in seen_urls:
                return
            seen_urls.add(resp.url)
        output_handler(resp, args, cli_logger, file_logger)
...
def collect_targets(target_input, args, protocols=['https', 'http'], ports=[80, 443], pages=["/"]):
    tmp = []
...
def main(args, cli_logger, file_logger):
    seen_urls = set()
    for url in collect_targets(args.target[0], args, args.proto, args.port, args.page):
        try:
            Thread(target=launcher, args=(url, args, seen_urls, cli_logger, file_logger), daemon=True).start()
...
def build_parser():
    ...
    parser.add_argument('-t', dest='timeout', type=int, default=4, help='Connection timeout')
    parser.add_argument('-v', dest="verbose", action='store_true', help='Show full responses')
    parser.add_argument('-T', dest='max_threads', type=int, default=45, help='Max threads (Default: 5)')
    parser.add_argument('-o', dest='outfile', action='store', help='Filename to write results (optional)')

    parser.add_argument('--port', default='', type=lambda x: delimiter2list(x), help="Define target port(s)")
    parser.add_argument('--page', default='/', type=lambda x: delimiter2list(x), help="Define request page (default /)")
    parser.add_argument('-H', '-header', dest="header", type=str, default=False, help="Extract header(s) from response, i.e: Server,X-Powered-By")
    parser.add_argument("--hide-title", action='store_false', help="Hide title output from CLI, useful to only extract headers.")
    ...
    parser.add_argument('--proto', default='http,https', type=lambda x: delimiter2list(x), help='Define protocol (http,https*)')
    parser.add_argument('--resolve', action='store_true', help='Resolve all DNS names before enumeration')
    parser.add_argument(dest='target', nargs='+', help='Target')
    return parser
...
def cli(argv=None):
    ...
    cli_logger = setup_cli_logger(spacer=[4, 45, 30])
