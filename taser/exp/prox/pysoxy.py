...
+ MACOS ISSUES WITH SO_BINDTODEVICE
    Need to ADD 2 DEFAULT ROUTEs
...
def connect_to_dst(dst_addr, dst_port):
    ...
    if OUTGOING_INTERFACE:
        sock = create_socket()
        try:
            if sys.platform == "darwin":        # CHECK IF MAC
                IP_BOUND_IF = 25
                sock.setsockopt(socket.IPPROTO_IP, IP_BOUND_IF, socket.if_nametoindex(OUTGOING_INTERFACE))
            else:
                sock.setsockopt(
                    socket.SOL_SOCKET,
